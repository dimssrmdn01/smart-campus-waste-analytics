...
st.set_page_config(page_title="Smart Campus Waste Logistics Ultra-Pro", layout="wide", page_icon="♻️")

np.random.seed(24)

# ==========================================
# 1. ENGINE DATABASE SQLITE (SQL PERSISTENCE)
# ==========================================
def init_db():
    conn = sqlite3.connect('campus_waste.db')
    cursor = conn.cursor()
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS log_pengangkutan (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            timestamp TEXT,
            titik_kritis INTEGER,
            jarak_dihemat REAL,
            co2_dihemat REAL
        )
    ''')
...
def simpan_log_ke_sql(titik_kritis, jarak, co2):
    conn = sqlite3.connect('campus_waste.db')
    cursor = conn.cursor()
    waktu_sekarang = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    cursor.execute('''
        INSERT INTO log_pengangkutan (timestamp, titik_kritis, jarak_dihemat, co2_dihemat)
        VALUES (?, ?, ?, ?)
    ''', (waktu_sekarang, titik_kritis, jarak, co2))
    conn.commit()
    conn.close()

def ambil_history_dari_sql():
    conn = sqlite3.connect('campus_waste.db')
    df = pd.read_sql_query("SELECT * FROM log_pengangkutan ORDER BY id ASC", conn)
    conn.close()
    return df

# Inisialisasi skema tabel SQL saat aplikasi pertama kali dimuat
init_db()

# ==========================================
# 2. SETUP DATA SPASIAL UTAMA (CAMPUS GRID)
# ==========================================
...
st.title("♻️ Smart Campus Waste Analytics - Ultra Pro (SQL Database Integrated)")
st.subheader("Sistem Optimasi Spasial, Inteligensi Prediktif, & Manajemen Data Persisten SQL")
st.write("---")
...
jarak_konvensional = sum([matriks_jarak[i][(i+1)%n_titik] for i in range(n_titik)])
...
co2_dihemat = (jarak_dihemat / 1000) * 268.5

# ==========================================
# 5. RENDER UTAMA INTERFACE & KPI METRICS
# ==========================================
...
with m2:
    st.metric(label="🌱 Emisi Karbon Dicegah", value=f"{round(co2_dihemat, 1)} Gram CO₂", delta="Dampak ESG Positif")
with m3:
    st.metric(label="🚨 Titik Kritis Pengangkutan", value=f"{len(titik_wajib_angkut)-1} / {n_titik-1} Lokasi")
...
with col1:
    ...
    # INTERAKSI DATABASE: Tombol Commit ke SQL
    st.markdown("#### 🔒 Gatekeeper Logistik")
    if st.button("💾 Simpan Log Shift Pengangkutan ke SQL"):
        simpan_log_ke_sql(len(titik_wajib_angkut)-1, round(jarak_dihemat, 2), round(co2_dihemat, 2))
        st.success("Berhasil mengunci data shift dan melakukan komit ke `campus_waste.db`!")
...
# ==========================================
# 6. HISTORICAL ESG TRACKING FROM SQL TABLE
# ==========================================
st.write("---")
st.markdown("### 📈 Evaluasi Dampak Lingkungan Historis (Live Data Query dari Database SQL)")

df_history = ambil_history_dari_sql()

if not df_history.empty:
    h1, h2 = st.columns([1, 2])
    with h1:
        st.markdown("**5 Log Aktivitas Terakhir (Tabel SQL):**")
        st.dataframe(df_history[['timestamp', 'titik_kritis', 'co2_dihemat']].tail(5), use_container_width=True)
    with h2:
        st.markdown("**Akumulasi Tren Pencegahan Emisi Karbon ($CO_2$):**")
        st.line_chart(data=df_history, x='timestamp', y='co2_dihemat', use_container_width=True)
else:
    st.warning("Database SQL masih kosong. Silakan klik tombol 'Simpan Log Shift Pengangkutan ke SQL' di atas untuk menyuntikkan data pertama.")